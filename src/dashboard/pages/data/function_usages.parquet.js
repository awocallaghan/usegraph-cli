/**
 * Observable Framework data loader: function_usages.parquet.js
 *
 * Proxies ~/.usegraph/built/function_usages.parquet to the browser.
...
const srcFile = join(getBuiltDir(), 'function_usages.parquet');

if (existsSync(srcFile)) {
  process.stdout.write(readFileSync(srcFile));
} else {
  const tmpFile = join(tmpdir(), `usegraph_empty_fu_${Date.now()}.parquet`);
