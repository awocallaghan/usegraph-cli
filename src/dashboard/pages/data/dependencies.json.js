...
const [topPackagesRaw, allDeps, prereleaseExposure, internalPackages] = await Promise.all([
  ...
  queryParquet(
    `SELECT package_name,
            ...
            COUNT(DISTINCT CASE WHEN dep_type = 'devDependencies' THEN project_id END)::INTEGER AS dev_count
     FROM ${deps}
     WHERE is_latest = true AND is_internal = true
     GROUP BY package_name
     ORDER BY project_count DESC`,
  ),
]);
...
process.stdout.write(
  JSON.stringify({ topPackages, allDeps, prereleaseExposure, internalPackages }),
);
