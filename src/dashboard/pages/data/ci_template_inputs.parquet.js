/**
 * Observable Framework data loader: ci_template_inputs.parquet.js
 *
 * Proxies ~/.usegraph/built/ci_template_inputs.parquet to the browser.
...
const srcFile = join(getBuiltDir(), 'ci_template_inputs.parquet');

if (existsSync(srcFile)) {
  process.stdout.write(readFileSync(srcFile));
} else {
  const tmpFile = join(tmpdir(), `usegraph_empty_citi_${Date.now()}.parquet`);
